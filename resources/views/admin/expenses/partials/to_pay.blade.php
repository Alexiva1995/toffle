<div class="row align-items-center">
    <div class="col-12 col-md-6">
        <div class="row my-3">
            ...
            <div class="col-auto">
                <h5>Monto Total por Pagar: <span class="h6"> {{ $expenses->where('status', 0)->sum('amount') }} </span> </h5>
            </div>
        </div>
    </div>
</div>
<div class="table-responsive">
    <table class="table" id="table_orders_to_pay">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($expenses->where('status', 0)  as $expense)
            <tr>
                ...
                <td class="text-center">
                    ...
                    <button class="btn btn-sm btn-danger" onclick="deleteElement( {{ $expense->id }}, '#delete_expense_to_pay_', 'este Gasto' )"> 
                        <i data-feather="trash-2"></i> 
                    </button>
                    <form id="delete_expense_to_pay_{{ $expense->id }}" action="{{ route('expenses.destroy', $expense->id) }}" method="POST">
                        @csrf
                        @method('DELETE')   
