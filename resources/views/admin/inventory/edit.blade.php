    ...
    <div class="col-12">
        <div class="card m-0">
            ...
            <div class="card-body p-0 px-2">
                <form class="form form-vertical" action="{{ route('inventory.store') }}" id="form_edit_inventory" method="POST">
                    @csrf
                    @method('PATCH')
                            ...
                            <div class="mb-1">
                                ...
                                <select class="select2 form-control" data-toggle="select" name="product_id"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"> {{ $product->name }} - Marca: {{ $product->mark }} </option>
                                    @endforeach
                                </select>
                                @error('product_id')
